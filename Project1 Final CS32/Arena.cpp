...
Arena::Arena(int nRows, int nCols)
	: m_rows(nRows), m_cols(nCols), m_player(nullptr), m_nZombies(0), hist(nRows, nCols)
{
...
bool Arena::attackZombieAt(int r, int c, int dir){
	...
	{
		delete m_zombies[k];
		m_zombies[k] = m_zombies[m_nZombies - 1]; //in spot k, put the last zombie there
		m_nZombies--;
		hist.record(m_player->row(), m_player->col()); //record where the player was
		return true;
	}
	return false;
}
